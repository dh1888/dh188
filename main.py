...
async def auto_end_current_activity(
    ...
    triggered_by: str = "activity",  # 🆕 新增参数：触发类型
):
    """自动结束当前活动"""
    try:
        ...
        # ✅ 根据触发类型使用不同的 checkin_type
        if triggered_by == "work_end":
            # 下班触发的自动结束，用 work_end 判定
            checkin_type = "work_end"
        else:
            # 主动回座或其他情况，用 activity 判定
            checkin_type = "activity"

        shift_info = await db.determine_shift_for_time(
            chat_id=chat_id,
            current_time=now,
            checkin_type=checkin_type,  # 使用正确的类型
        )
        ...
        if shift_info:
            ...
            logger.info(
                f"📅 自动结束活动 - 触发类型: {triggered_by}, "
                f"班次判定: {shift_info.get('shift_detail')}, "
                f"记录日期: {forced_date}"
            )
        else:
        ...
        # ========== 2. 下班打卡 ==========
            ...
            if current_activity:
                with suppress(Exception):
                    await auto_end_current_activity(
                        chat_id, uid, user_data, now, message, triggered_by="work_end"
                    )
...
async def initialize_services():
    ...
    try:
        ...
        logger.info(
            ...
            f"   └─ 导出时间: 每月1号 {getattr(Config, 'MONTHLY_EXPORT_HOUR', 15):02d}:{getattr(Config, 'MONTHLY_EXPORT_MINUTE', 0):02d}"
        )
