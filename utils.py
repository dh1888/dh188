...
async def calculate_fine(activity: str, overtime_minutes: float) -> int:
    """计算活动超时罚款（委托 database 统一实现）"""
    return await db.calculate_fine_for_activity(activity, overtime_minutes)
...
class ActivityTimerManager:
    ...
    async def _activity_timer_wrapper(
        ...
        try:
            from activity_service import activity_timer
...
async def send_reset_notification(
    ...
    try:
        ...
        if completion_result.get("export_success") is not None:
            export_ok = completion_result["export_success"]
            notification_text += (
                f"\n📤 数据导出: {'✅ 成功' if export_ok else '❌ 失败'}"
            )

        if completion_result.get("cleanup_skipped"):
            notification_text += (
                "\n🛡️ 物理清理: ⏭️ 已跳过（导出失败，数据已保留）"
            )

        cleanup = completion_result.get("cleanup")
        if cleanup:
            deleted_total = sum(
                v for k, v in cleanup.items() if isinstance(v, int) and k != "users_reset"
            )
            if deleted_total:
                notification_text += f"\n🧹 清理记录: <code>{deleted_total}</code> 条"

        day_shift = completion_result.get("day_shift")
        night_shift = completion_result.get("night_shift")
        if day_shift or night_shift:
            notification_text += "\n\n📋 <b>班次统计:</b>"
            if day_shift:
                notification_text += (
                    f"\n☀️ 白班: 强制结束 {day_shift.get('forced', 0)} / "
                    f"补全下班 {day_shift.get('completed', 0)}"
                )
            if night_shift:
                notification_text += (
                    f"\n🌙 夜班: 强制结束 {night_shift.get('forced', 0)} / "
                    f"补全下班 {night_shift.get('completed', 0)}"
                )

        if completed_count > 0 and details:
            notification_text += f"\n📋 <b>活动结束详情:</b>\n"
